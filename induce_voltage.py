...
def calculate(xp, yp, obj_size):
    ...
    with np.errstate(divide='ignore',invalid='ignore'):
        if case == 1:  # case 1: 115kv
            ...
            Matrix = np.array(
                [[np.log(2 * ya / r_115), np.log(np.divide(distantp['ab'] , distant['ab'])), np.log(np.divide(distantp['ac'] , distant['ac']))],
                [np.log(np.divide(distantp['ab'] , distant['ab'])), np.log(2 * yb / r_115), np.log(np.divide(distantp['bc'] , distant['bc']))],
                [np.log(np.divide(distantp['ac'] , distant['ac'])), np.log(np.divide(distantp['bc'] , distant['bc'])), np.log(2 * yc / r_115)]])
            q_cart = (2 * np.pi * EPSILON_0) * np.matmul(np.linalg.inv(Matrix), v_complex)
            vpe = ((q_cart[0] * np.log(np.divide(distantp['pa'] , distant['pa']))) + (
            ...
            SuperPosition = np.array([[np.log(np.divide(distantD['pa'] , distant['pa'])), np.log(np.divide(distantD['pb'] , distant['pb'])),np.log(np.divide(distantD['pc'] , distant['pc']))]])
